...
class Html
{
    ...
    protected static function parseNode($node, $element, $styles = array(), $data = array())
    {
        ...
        $nodes = array(
            ...
            //  'br'         => array('Paragraph',   $node,  $element,   $styles,    null,   null,           null),
            ...
            'img' => array('Img', $node, $element, $styles, $data, null, null),
        );
     ...
     * @param array &$styles
     */
    private static function parseImg($node, $element, &$styles, $argument1)
    {
        $newElement = null;
        $source = null;
        if ($node->nodeType == XML_ELEMENT_NODE) {
    ...
    private static function parseTable($node, $element, &$styles, $argument1)
    {
        ...
        switch ($argument1) {
            case 'addTable':
                $styles['paragraph'] = self::parseInlineStyle($node, $styles['paragraph']);
                $newElement = $element->addTable('table');
                break;
            case 'skipTbody':
                $newElement = $element;
                break;
            case 'addRow':
                /** @var TYPE_NAME $newElement */
                $newElement = $element->addRow();
                break;
            case 'addCell':
                $newElement = $element->addCell(10000);
                /** @var \PhpOffice\PhpWord\Element\Row $element */
                foreach($element->getCells() as  $item)
                {
