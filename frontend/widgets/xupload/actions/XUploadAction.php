<?php
namespace frontend\widgets\xupload\actions;
...
class XUploadAction extends Action
{
     ...
     * @since 0.5
     */
    public $formClass = 'frontend\widgets\xupload\models\XUploadForm';
            ...
            if ($model->validate()) {
                ...
                if ($returnValue === true) {
                    ...
                    try {
                        ...
                        if (preg_match('/\.(gif|jpe?g|png)$/i', $model->{$this->displayNameAttribute})) {
                            $image = \Gregwar\Image\Image::open($path . $model->{$this->fileNameAttribute});
