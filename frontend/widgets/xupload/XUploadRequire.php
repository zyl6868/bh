 ...
 *
 */
class XUploadRequire extends InputWidget
{
    ...
    {
        $baseUrl = BH_CDN_RES.'/static';
        // $this->registerJsFile($baseUrl . '/js/jquery.ui.widget.js', [ 'position'=>\yii\web\View::POS_HEAD] );
        \Yii::$app->view->registerCssFile($baseUrl . '/js/lib/jqueryfileupload/jquery.fileupload.css');

//        // The basic File Upload plugin
//        \Yii::$app->view->registerJsFile($baseUrl . '/js/lib/jqueryfileupload/jquery.fileupload.js', ['position' => View::POS_HEAD]);
//
//        //The Iframe Transport is required for browsers without support for XHR file uploads
//        \Yii::$app->view->registerJsFile($baseUrl . '/js/lib/jqueryfileupload/jquery.iframe-transport.js', ['position' => View::POS_HEAD]);
    ...
    public function run()
    {
        ...
        if (isset($this->htmlOptions["class"])) {
            \Yii::$app->view->registerJs( "require(['lib/jqueryfileupload/jquery.iframe-transport','lib/jqueryfileupload/jquery.fileupload-process','lib/jqueryfileupload/jquery.fileupload-validate'], function(){ $(\".{$this->htmlOptions['class']}:input[type='file']\").fileupload({$options});  });", View::POS_END,__CLASS__ . 'class' . $this->htmlOptions['class']);

        } else {
            \Yii::$app->view->registerJs("require(['lib/jqueryfileupload/jquery.iframe-transport','lib/jqueryfileupload/jquery.fileupload-process','lib/jqueryfileupload/jquery.fileupload-validate'], function(){ $('#{$this->htmlOptions['id']}').fileupload({$options});  });", View::POS_END,__CLASS__ . '#' . $this->htmlOptions['id']);
