<?php
namespace  frontend\widgets\xupload;
use yii\web\View;
...
class XUpload extends InputWidget {
    ...
    public function publishAssets() {
        ...
        if (is_dir($assets)) {
            ...
            Yii::app() -> clientScript -> registerScriptFile($baseUrl . '/js/tmpl.min.js', ['position'=>View::POS_END]);
            // The basic File Upload plugin
            Yii::app() -> clientScript -> registerScriptFile($baseUrl . '/js/jquery.fileupload.js', ['position'=>View::POS_END]);
            if($this->previewImages || $this->imageProcessing){
                Yii::app() -> clientScript -> registerScriptFile($baseUrl . '/js/load-image.min.js', ['position'=>View::POS_END]);
                Yii::app() -> clientScript -> registerScriptFile($baseUrl . '/js/canvas-to-blob.min.js', ['position'=>View::POS_END]);
            }
            //The Iframe Transport is required for browsers without support for XHR file uploads
            Yii::app() -> clientScript -> registerScriptFile($baseUrl . '/js/jquery.iframe-transport.js', ['position'=>View::POS_END]);
            // The File Upload image processing plugin
            if($this->imageProcessing){
                Yii::app() -> clientScript -> registerScriptFile($baseUrl . '/js/jquery.fileupload-ip.js', ['position'=>View::POS_END]);
            }
            //The File Upload user interface plugin
            Yii::app() -> clientScript -> registerScriptFile($baseUrl . '/js/jquery.fileupload-ui.js', ['position'=>View::POS_END]);
            ...
            Yii::$app->clientScript->registerScript('XuploadI18N', $js, ['position'=>View::POS_END]);
            /**
    ...
    {
        return Yii::t('xupload.widget', $message, $params, null, 'en_us');
    }
    ...
    public function run()
    {
        ...
        Yii::$app->clientScript->registerScript(__CLASS__ . '#' . $this->htmlOptions['id'], "jQuery('#{$this->htmlOptions['id']}').fileupload({$options});", ['position'=>View::POS_READY]);
        $htmlOptions = array();
        if ($this->multiple) {
