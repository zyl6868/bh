...
class HomeworkunfinishController extends ClassesBaseController
{
    ...
    public function actionIndex(int $classId)
    {
        ...
        foreach($classHomeworkReport as $report){
            ...
            $finishArr[] = $report->finishNum;
            $unfinishArr[] = $report->totalNum - $report->finishNum;
        }

        if(app()->request->isAjax){
            return $this->renderPartial('_index_info',['subjectArr'=>$subjectArr ,'finishArr'=>$finishArr , 'unfinishArr'=>$unfinishArr]);
        }


        return $this->render('index',['classId'=>$classId ,'years'=>$years,'months'=>$months , 'subjectArr'=>$subjectArr ,'finishArr'=>$finishArr , 'unfinishArr'=>$unfinishArr,'firstDay'=>$firstDay]);
    }
