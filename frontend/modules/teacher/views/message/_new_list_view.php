...
<div class="tab_1 tab main_h4">
    <ul>
        <?php if (empty($modelList->list)) {
            ViewHelper::emptyViewByPage($pages);
        } else {
            if ($category == 2) {
            foreach ($modelList->list as $key => $val) {

                    ?>
                    <li data-msgid="<?php echo $val->messageID;?>">
                        <h4 <?php if($val->isRead == 0){echo 'class="font_bold"';}?>><span class="receiver" infoid="<?php echo $val->messageID;?>" title="<?php echo Html::encode($val->messageTiltle);?>"><?php echo cut_str($val->messageTiltle,30); ?></span><i class="cut delmes" delid="<?php echo $val->messageID;?>"></i></h4>
                        <p>From&nbsp;:&nbsp;<span><?php echo $val->sentName;?></span><span><?php echo $val->sentTime;?></span></p>
                    </li>
                <?php } ?>
            <?php }
        } ?>
