define(['jquery', "popBox", 'userCard', 'jquery_sanhai', 'echarts/echarts', 'validationEngine', 'validationEngine_zh_CN', 'jqueryUI', 'echarts/chart/pie'],
    ...
    function ($, popBox, userCard, jquery_sanhai, ec, validationEngine, validationEngine_zh_CN) {
        $('#ComboBox>ul li:last-child').css('border','0');
        Element.prototype.Trim = function () {
            if(this.innerHTML.replace(/(^\s*)|(\s*$)/g, "").length>26){
                this.innerHTML = this.innerHTML.replace(/(^\s*)|(\s*$)/g, "").slice(0, 26) + '...';
            }
        };
        var $work_title = document.querySelectorAll('.work_title');
        for (var i = 0, len = $work_title.length; len > i; i++) {
            $work_title[i].Trim();
        }
        $(document).on('mouseover', '#ComboBox', function () {
            var $this = $(this);
            $this.children('div').css({'border':'1px solid #ccc','border-bottom':'0'});
            $this.children('ul').show();
            $this.find('i').attr('class', 'top');
        }).on('mouseout', '#ComboBox', function () {
            var $this = $(this);
            $this.children('div').css('border','0');
            $this.children('ul').hide();
            $this.find('i').attr('class', 'bottom');
        }).on('mouseover', '#ComboBox ul li', function () {
            $(this).addClass('blue');
        }).on('mouseout', '#ComboBox ul li', function () {
            $(this).removeClass('blue');
        }).on('click', '#ComboBox ul li', function () {
            $('#ComboBox span').html($(this).html());
            $('#ComboBox ul').hide();
        }).on('click', '#MyError button.icoBtn_open', function () {
            var $this = $(this);
        ...
        (function () {
            ...
            $('.answer_questions_btn').live('click', function () {
                ...
                if (answer.length > 1001) {
                    ...
                    $.post('/answernew/result-question', {
                        answer: answer,
                        aqid: aqid,
                        img_val: img_val
                    }, function (data) {
