<template>
  <div class="wrapper">
    ...
    <div class="fineWorkBox" ref="fineWorkBox" v-show="coursewareLists.length > 0">
      <mt-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore" @bottom-status-change="handleBottomChange" :auto-fill="isAutoFill">
        <ul class="fineWorkList">
          <li class="fineWorkCtn" v-for="lists in coursewareLists" v-tap="{methods: toHomework, listId: lists.id}">
...
export default {
  ...
  mounted() {
    this.$refs.fineWorkBox.style.height = (document.body.clientHeight - this.$refs.selectPackage.offsetHeight - 20) + 'px';
  },
  methods: {
    ...
    loadBottom() { // 下拉加载更多操作
      ...
      if (this.pageIndex > this.pageCount) {
        ...
        this.$axios.get(this.$httpConfig.hwList, {
          params: {
            ...
            'homework-type': this.twoSelect[1].id,
            'page': this.pageIndex++
          }
        })
          .then(res => {
            if (res.status == 200) {
              ...
              this.coursewareLists = this.coursewareLists.concat(res.data.items);
            } else {
              Bus.$emit('message', res.data.message);
            }
          })
