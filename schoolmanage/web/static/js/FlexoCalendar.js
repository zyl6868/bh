...
+(function($){
	...
	Calendar.prototype.init = function(element, options){
		...
		var oldOptions = this.opt = this.getOptions( options, this.date );
		this.type = oldOptions.type;

		if( this.type == 'normal' ){
			this.date = this.getDate1();
			var options = this.opt = this.getOptions( options, this.date );
			this.type = options.type;
		}
		else if(this.type == 'weekly' ){
			this.date = this.getDate2();
			var options = this.opt = this.getOptions( options, this.date );
			this.type = options.type;
		}
		else if(this.type == 'monthly' ){
			this.date = this.getDate3();
			var options = this.opt = this.getOptions( options, this.date );
			this.type = options.type;
		}
	...
	Calendar.prototype.bulidFull = function(options, year, month){
		...
		var optionsNow = new Date(new Date() - 1000*60*60*24);
		var optionsRealyear = optionsNow.getFullYear();
		var optionsRealmonth = optionsNow.getMonth() + 1;
		var optionsCrtdate = optionsNow.getDate();
		for(var j = 0; j < 42; j++){
			if( j % 7 == 0 && j != 41 ){
				if( j != 0){
					calendar +="</tr><tr>";
			...
			if( j >= firDay && ( j < (firDay + monthLen)) ){
				if( options.today ){
					...
					//if( j == parseInt(optionsCrtdate) + firDay - 1 && optionsRealyear == year && parseInt(optionsRealmonth) == month){
	...
	Calendar.prototype.bulidWeekly = function(options, year ,month){
		...
		var crt;
		var optionsNow = new Date(new Date() - 1000*60*60*24*7);
		var optionsRealyear = optionsNow.getFullYear();
		var optionsRealmonth = optionsNow.getMonth() + 1;
		var optionsCrtdate = optionsNow.getDate();
		for(var j = 0; j < 5; j++ ){
			...
			//第一个else if是原版本，第二个else if是改成上一周
			...
			//else if( options.today && ( (parseInt(optionsRealmonth) == startMonth && startDay <= parseInt(optionsCrtdate)) || ( parseInt(optionsRealmonth) != startMonth && startDay >= parseInt(optionsCrtdate)) ) && parseInt(optionsCrtdate) <= endDay && optionsRealyear == year && optionsRealmonth == month){
				if( selectDate ){
					crt = 'week' + ( j + 1 );
	...
	Calendar.prototype.bulidMonthly = function(options, year ,month){
		...
		var optionsNow = new Date();
		optionsNow.setMonth(optionsNow.getMonth() - 1);
		var optionsRealyear = optionsNow.getFullYear();
		var optionsRealmonth = optionsNow.getMonth() + 1;
		for(var j = 0; j < 12; j++){
			if( j % 4 == 0 && j !=12 ){
			...
			//if( options.today && (j + 1) == optionsRealmonth && optionsRealyear == year){
	...
	Calendar.prototype.getDate1 = function(){
		var now = new Date(new Date() - 1000*60*60*24);
		var crtdate = now.getDate();
		var setMonth = now.getMonth() + 1 ;
		var setYear = now.getFullYear();
		crtdate = this.dispose(crtdate);
	...
	Calendar.prototype.getDate2 = function(){
		var now = new Date(new Date() - 1000*60*60*24*7);
		var crtdate = now.getDate();
		var setMonth = now.getMonth() + 1 ;
		var setYear = now.getFullYear();
		crtdate = this.dispose(crtdate);
	...
	Calendar.prototype.getDate3 = function(){
		var now = new Date();
